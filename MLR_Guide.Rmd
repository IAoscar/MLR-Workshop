...
Build model
```{r}
train_model <- lm(Performance.Index ~ Hours.Studied + Previous.Scores + Extracurricular.Activities + Sleep.Hours + Sample.Question.Papers.Practiced, data = train_df)
...
```{r}
backward_model <- step(train_model, direction = "backward")
...
Predict values
```{r}
predictions <- predict(train_model, newdata = test_df)
...
```{r}
singleStudent <- data.frame(Hours.Studied = 6, Previous.Scores = 75, Extracurricular.Activities = 0, Sleep.Hours = 7.5, Sample.Question.Papers.Practiced = 2)

print(singleStudent)

predicted_value <- predict(train_model, newdata= singleStudent)
...
```{r}
absDiff <- abs(results_df$actual-results_df$predictions)
...
```{r}
accuracyWithinX <- ifelse(abs(results_df$predictions-results_df$actual) < 0.10*results_df$actual,1,0)
